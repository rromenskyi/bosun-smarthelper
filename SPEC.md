...
- [x] Alerts (`internal/alerts`, `docs/alerts.md`) — "something is wrong enough a human should hear about it right now": NOAA weather alerts (`api.weather.gov`, no API key, US-only coverage) for the current position (fixed lat/lon, or the `get_gps` tool's live reading on every tick), and any `internal/metrics`-sampled metric (`metrics.sources`) crossing a configured threshold — metric-name-agnostic by design, so a future battery-charge or grey/black/fresh-water-tank sensor needs only a new `metrics.sources` entry and a new threshold, no code change. Both sources are edge-triggered (notify once on a state transition, not on every tick a threshold stays crossed). Delivered through Telegram, a webhook, and/or the host's own speaker (via the same Piper TTS engine voice replies use) — each channel fires only once it's both configured in `config.yaml`/`.env` and turned on from the settings page's "Алерты"/"Alerts" tab, the same config-vs-settings split the backup auto-schedule already uses
