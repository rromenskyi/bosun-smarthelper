...
- [x] Manual backup to any S3-compatible bucket (`internal/backup`, `smarthelper backup`, `docs/backup.md`) — archives persistent data, dumping `metrics.db` to portable SQL rather than copying the live file raw; a hand-rolled SigV4 signer (verified live against a real MinIO server) instead of the AWS SDK, since the feature needs exactly one PUT request. Deliberately no scheduled/automatic variant — bandwidth is never spent without being explicitly asked for
