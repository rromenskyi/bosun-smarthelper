...
- [x] LLM-free quick-access chips (`internal/webui/quick.go`, `GET /api/quick/{tool}`) — System status and GPS bypass the LLM entirely for a plain numeric readout; caught live, repeatedly, a weak local model skipping the tool call and instead copying a stale reading out of its own session history. Never written to session history either, so it can't leave a stale reading behind for a later question to copy
- [x] Backup/restore to any S3-compatible bucket (`internal/backup`, `smarthelper backup`/`restore`, `docs/backup.md`) — archives persistent data, dumping `metrics.db` to portable SQL rather than copying the live file raw; `restore` extracts into a fresh directory (never the live one) and replays the SQL automatically. A hand-rolled SigV4 signer (Put/Get/List, verified live against both a local MinIO server and a real Backblaze B2 bucket end-to-end) instead of the AWS SDK. Manual by default (`smarthelper backup`, or the settings page's "back up now") — an optional settings-page toggle turns on an interval-based automatic schedule (daily/weekly/monthly) instead, off unless explicitly enabled, checked via the same idle-tick discipline (`server.TryIdleAfter`) every other background pass uses
- [x] Voice devices (`internal/webui/device.go`, `docs/devices.md`) — WebSocket endpoint for hardware voice front-ends (ESP32 speaker); push-to-talk turns through the existing STT/agent/TTS; streaming TTS and alert announcements on devices still to come
- [x] Alerts (`internal/alerts`, `docs/alerts.md`) — "something is wrong enough a human should hear about it right now": NOAA weather alerts (`api.weather.gov`, no API key, US-only coverage) for the current position (fixed lat/lon, or the `get_gps` tool's live reading on every tick), and any `internal/metrics`-sampled metric (`metrics.sources`) crossing a threshold — metric-name-agnostic by design, so a future battery-charge or grey/black/fresh-water-tank sensor needs only a new `metrics.sources` entry, no code change. Both sources are edge-triggered (notify once on a state transition, not on every tick a threshold stays crossed). NOAA channels are a global on/off per channel from the settings page; threshold *rules* are entirely web-managed (`settings.Data.AlertsThresholds` — `config.yaml`'s `alerts.thresholds` only ever seeds it once), each with its own independent Telegram/webhook/speaker selection, optional moving-average smoothing over the last N samples, custom alarm text sent to every channel it enables, and a per-rule "siren before the text" checkbox (a built-in sound, `internal/alerts/assets/siren.wav`, embedded via `go:embed`) for the speaker channel. A channel still only fires once it's configured in `config.yaml`/`.env`, the same config-vs-settings split the backup auto-schedule uses
- [x] `run_code` sandbox (`internal/tools/codeexec.go`, `internal/sandbox`, `docs/sandbox.md`) — the LLM can write and run a short Python 3 program for math/parsing/simulation it would otherwise reason about badly. Never executed inside `bosun` itself: a separate `sandboxd` service (its own container, the only one holding `/var/run/docker.sock`) runs each session's code in its own long-lived Docker container, reused across a multi-step task and reaped after an idle TTL, not torn down immediately. Full LAN/internet access from the executed code, by deliberate choice — this is the one exception to the "tool sandboxing" non-goal below, and the isolation goal is narrower than a multi-tenant sandbox: only that the code can never escape to the real host, not that it's isolated from the rest of this trusted personal LAN. `session_id` is never LLM-supplied — it's the real webui chat session ID threaded through `context.Context`, so the model can't forge or collide workspaces. Off by default, and needs two separate opt-ins to actually run at all (`sandbox.enabled` in `config.yaml`, and `COMPOSE_PROFILES=sandbox` in `.env`), since the service itself is root-equivalent the moment it exists regardless of the app-level toggle
- [x] Cameras (`internal/cameras`, `docs/cameras.md`) — a 📹 button for one or more WiFi cameras' live view and recorded archive. These cameras typically accept only one client at a time; `internal/cameras.Relay` is the only thing that ever connects to a camera directly, republishing every frame to any number of subscribers instead — live browser viewers via `GET /api/cameras/{name}/stream` (native `multipart/x-mixed-replace`, no player library) and an `ffmpeg`-based cyclic (ring-buffer, `-segment_wrap`) recorder that itself reads from the relay, not the camera. Runs inside `bosun` itself (already has `ffmpeg` for voice input) rather than a third-party restreamer (go2rtc et al.) — hand-rolled for the same reason S3 signing and the sandbox's Docker-CLI wrapper were: MJPEG multipart relaying is a small, well-defined protocol. Recordings land outside `cfg.Backup.DataDir`, excluded from the S3 backup by construction. Config-only (like `metrics.sources`), not settings-page-editable — which cameras exist is an infrastructure decision
...
- Tool sandboxing for the built-in sensor/API tools (they run as the invoking `bosun` process, same as always) — the one deliberate exception is `run_code` (`internal/tools/codeexec.go`, `docs/sandbox.md`), which executes arbitrary LLM-authored code and specifically needs isolation the other tools don't
